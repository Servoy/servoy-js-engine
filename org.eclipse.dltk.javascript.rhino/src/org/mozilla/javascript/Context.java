...
public class Context {
	 ...
	 * default behavior of {@link #hasFeature(int)} is always to subtruct 1900
	 * as rquired by ECMAScript B.2.4.
	 */
	public static final int FEATURE_NON_ECMA_GET_YEAR = 1;

	/**
	 * Control if member expression as function name extension is available. If
	 * <tt>hasFeature(FEATURE_MEMBER_EXPR_AS_FUNCTION_NAME)</tt> returns true,
	 * allow <tt>function memberExpression(args) { body }</tt> to be syntax
	 * sugar for <tt>memberExpression = function(args) { body }</tt>, when
	 * memberExpression is not a simple identifier. See ECMAScript-262, section
	 * 11.2 for definition of memberExpression. By default
	 ...
	 * future reserved keyword (see Ecma-262, section 7.5.3) as ordinary
	...
	/**
	 * Control if properties <tt>__proto__</tt> and <tt>__parent__</tt> are
	 * treated specially. If
	 * <tt>hasFeature(FEATURE_PARENT_PROTO_PROPERTIES)</tt> returns true, treat
	 * <tt>__parent__</tt> and <tt>__proto__</tt> as special properties.
	 * <p>
	 * The properties allow to query and set scope and prototype chains for the
	 * objects. The special meaning of the properties is available only when
	 * they are used as the right hand side of the dot operator. For example,
	 * while <tt>x.__proto__ = y</tt> changes the prototype chain of the object
	 * <tt>x</tt> to point to <tt>y</tt>, <tt>x["__proto__"] = y</tt> simply
	 * assigns a new value to the property <tt>__proto__</tt> in <tt>x</tt> even
	 ...
	 * <p>
	 * By default {@link #hasFeature(int)} returns true if the current JS
	 * version is set to {@link #VERSION_DEFAULT} or is at least
	 * {@link #VERSION_1_6}.
	 * 
	 * @since 1.6 Release 1
	 */
	 ...
	 * hasFeature(FEATURE_DYNAMIC_SCOPE) returns true, then the name lookup
	 ...
	 * script or function from the current stack frame if the top scope of the
	 * top stack frame contains the top scope of the current stack frame on its
	 * prototype chain.
	 * <p>
	 * This is useful to define shared scope containing functions that can be
	 * called from scripts and functions using private scopes.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 1
	 ...
	 * reports runtime errors if assignment to a global variable that does not
	 * exist is executed. When the feature is off such assignments create a new
	 * variable in the global scope as required by ECMA 262.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 1
	...
	/**
	 * Control if strict eval mode is enabled. When the feature is on Rhino
	 * reports runtime errors if non-string argument is passed to the eval
	 * function. When the feature is off eval simply return non-string argument
	 * as is without performing any evaluation as required by ECMA 262.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 1
	 ...
	 * Error constructor. Note that neither behavior is fully ECMA 262 compliant
	 * (as 262 doesn't specify a three-arg constructor), but keeping the feature
	 * off results in Error objects that don't have additional non-ECMA
	 * properties when constructed using the ECMA-defined single-arg constructor
	 * and is thus desirable if a stricter ECMA compliance is desired,
	 * specifically adherence to the point 15.11.5. of the standard.
	 * <p>
	 * By default {@link #hasFeature(int)} returns false.
	 * 
	 * @since 1.6 Release 6
	 ...
	 * Rhino reports more than a dozen different warnings. When the feature is
	 * off, these warnings are not generated. FEATURE_STRICT_MODE implies
	...
	/**
	 * Enables enhanced access to Java. Specifically, controls whether private
	 * and protected members can be accessed, and whether scripts can catch all
	 * Java exceptions.
	 * <p>
	 ...
	 * 
	 * @param factory
	 *            the context factory associated with this context (most likely,
	 ...
	 * current thread and a Context if the prior call to
	 ...
	 * thread, then <tt>ContextFactory.getGlobal().makeContext()</tt> will be
	 * called to construct new Context instance. The instance will be temporary
	 ...
	 * using the Context instance associated with the current thread. If no
	 * Context is associated with the thread, then
	 * {@link ContextFactory#makeContext()} will be called to construct new
	 * Context instance. The instance will be temporary associated with the
	 * thread during call to {@link ContextAction#run(Context)}.
	 * <p>
	 * It is allowed but not advisable to use null for <tt>factory</tt> argument
	 * in which case the global static singleton ContextFactory instance will be
	 ...
	 * allow to modify any of its properties and will throw an exception on any
	 * such attempt.
	 * 
	 ...
	 * including calling {@link #enter()} and {@link #exit()} methods will throw
	 * an exception.
	 * <p>
	 * If <tt>sealKey</tt> is not null, calling {@link #unseal(Object sealKey)}
	 * with the same key unseals the object. If <tt>sealKey</tt> is null,
	 * unsealing is no longer possible.
	 * 
	 * @see #isSealed()
	 * @see #unseal(Object)
	 */
	...
	/**
	 * Unseal previously sealed Context object. The <tt>sealKey</tt> argument
	 * should not be null and should match <tt>sealKey</tt> suplied with the
	 * last call to {@link #seal(Object)} or an exception will be thrown.
	 * 
	 * @see #isSealed()
	 * @see #seal(Object sealKey)
	 */
	...
	public final ErrorReporter setErrorReporter(ErrorReporter reporter) {
		...
		Object listeners = propertyListeners;
		if (listeners != null) {
			firePropertyChangeImpl(listeners, errorReporterProperty, old,
					reporter);
		}
	...
	/**
	 * Register an object to receive notifications when a bound property has
	 * changed
	 * 
	 ...
	 * String, Number, Date, etc.), setting up 'scope' to act as a global object
	 * as in ECMA 15.1.
	 * <p>
	 * 
	 ...
	 * 
	 * @param scope
	 *            the scope to initialize, or null, in which case a new object
	 ...
	 * among several top-level objects. Note that sealing is not allowed in the
	 ...
	 * 
	 * The provided source name and line number are used for error messages and
	 * for producing debug information.
	 * 
	 * @param scope
	 ...
	 *            an arbitrary object that specifies security information about
	 *            the origin or owner of the script. For implementations that
	 *            don't care about security, this value may be null.
	 ...
	 * Caller must be prepared to catch a ContinuationPending exception and
	 * resume execution by calling
	 * {@link #resumeContinuation(Object, Scriptable, Object)}.
	 * 
	 * @param script
	 *            The script to execute. Script must have been compiled with
	 *            interpreted mode (optimization level -1)
	 * @param scope
	 *            The scope to execute the script against
	 * @throws ContinuationPending
	 *             if the script calls a function that results in a call to
	 ...
	 * 
	 * @param function
	 *            The function to call. The function must have been compiled
	 ...
	 * This implies that the code calling this method must have been called as a
	 * function from the JavaScript script. Also, there cannot be any
	 * non-JavaScript code between the JavaScript frames (e.g., a call to
	 ...
	 * {@link #captureContinuation()}. Execution of the code will resume with
	 * the functionResult as the result of the call that captured the
	 * continuation. Execution of the script will either conclude normally and
	 * the result returned, another continuation will be captured and thrown, or
	 * the script will terminate abnormally and throw an exception.
	 * 
	 * @param continuation
	 *            The value returned by
	 *            {@link ContinuationPending#getContinuation()}
	 * @param functionResult
	 *            This value will appear to the code being resumed as the result
	 *            of the function that captured the continuation
	 * @throws ContinuationPending
	 *             if another continuation is captured before the code
	 *             terminates
	 * @since 1.7 Release 2
	 */
	public Object resumeContinuation(Object continuation, Scriptable scope,
		...
		return Interpreter.restartContinuation(
				(org.mozilla.javascript.NativeContinuation) continuation, this,
				scope, args);
	}
	 ...
	 * JavaScript. If compiling the string would result in an error that might
	 * be fixed by appending more source, this method returns false. In every
	 * other case, it returns true.
	 * <p>
	 * Interactive shells may accumulate source lines, using this method after
	 * each new line is appended to check whether the statement being entered is
	 * complete.
	 * 
	 * @param source
	 ...
	 * <p>
	 * Returns a script that may later be executed. Will consume all the source
	 * in the reader.
	 * 
	 * @param in
	 *            the input reader
	 ...
	 *            a string describing the source, such as a filename
	 * @param lineno
	 *            the starting line number for reporting errors. Use 0 if the
	...
	final Script compileString(String source, Evaluator compiler,
		...
		try {
			return (Script) compileImpl(null, null, source, sourceName, lineno,
					securityDomain, false, compiler, compilationErrorReporter);
	 ...
	 * <p>
	 * The function source must be a function definition as defined by ECMA
	 * (e.g., "function f(a) { return a; }").
	 * 
	 * @param scope
	...
	final Function compileFunction(Scriptable scope, String source,
		...
		try {
			return (Function) compileImpl(scope, null, source, sourceName,
					lineno, securityDomain, true, compiler,
	 ...
	 * source.
	 * <p>
	 * Returns function body of '[native code]' if no decompilation information
	 * is available.
	 * 
	 * @param fun
	 ...
	 *            the scope to create the object in.
	 * @param elements
	 *            the initial elements. Each object in this array must be an
	 *            acceptable JavaScript type and type of array should be exactly
	 *            Object[], not SomeObjectSubclass[].
	 * @return the new array object.
	 */
	 ...
	 * then the number is converted Uint32 value as defined in Ecma 9.6 and Java
	 * array of that size is allocated. The array is initialized with the values
	 * obtained by calling get() on object for each value of i in [0,length-1].
	 * If there is not a defined value for a property the Undefined value is
	 * used to initialize the corresponding element in the array. The Java array
	 * is then returned. If the object doesn't define a length property or it is
	 * not a number, empty array is returned.
	 * 
	 * @param object
	 ...
	 * <p>
	 * Note that a scope must be provided to look up the constructors for
	 * Number, Boolean, and String.
	 * <p>
	 * See ECMA 9.9.
	 * <p>
	 * Additionally, arbitrary Java objects and classes will be wrapped in a
	 * Scriptable object with its Java fields and methods reflected as
	 * JavaScript properties of the object.
	 * 
	 * @param value
	 *            any Java object
	 * @param scope
	 *            global scope containing constructors for Number, Boolean, and
	 *            String
	 ...
	 * <tt>Number.doubleValue()</tt> resulting in a precision loss if the number
	 * can not fit into double.
	 * <p>
	...
	/**
	 * Convert a JavaScript value into the desired type. Uses the semantics
	 * defined with LiveConnect3 and throws an Illegal argument exception if the
	 * conversion cannot be performed.
	 * 
	 * @param value
	 *            the JavaScript value to convert
	 * @param desiredType
	 *            the Java type to convert to. Primitive Java types are
	 *            represented using the TYPE fields in the corresponding wrapper
	 *            class in java.lang.
	 ...
	 *             {@link #jsToJava(Object, Class)} throws
	...
	/**
	 * Rethrow the exception wrapping it as the script runtime exception. Unless
	 * the exception is instance of {@link EcmaError} or
	 * {@link EvaluatorException} it will be wrapped as {@link WrappedException}
	 * , a subclass of {@link EvaluatorException}. The resulting exception
	 * object always contains source name and line number of script that
	 * triggered exception.
	 * <p>
	 * This method always throws an exception, its return value is provided only
	 * for convenience to allow a usage like:
	 * 
	 * <pre>
	 ...
	 * negative values will be interpreted as -1, and any values greater than 9
	 * will be interpreted as 9. An optimization level of -1 indicates that
	 * interpretive mode will always be used. Levels 0 through 9 indicate that
	 * class files may be generated. Higher optimization levels trade off
	 * compile time performance for runtime performance. The optimizer level
	 * can't be set greater than -1 if the optimizer package doesn't exist at
	 * run time.
	 * 
	 * @param optimizationLevel
	 *            an integer indicating the level of optimization to perform
	 * @since 1.3
	 * 
	 */
	...
	/**
	 * Sets the maximum stack depth (in terms of number of call frames) allowed
	 * in a single invocation of interpreter. If the set depth would be
	 * exceeded, the interpreter will throw an EvaluatorException in the script.
	 * Defaults to Integer.MAX_VALUE. The setting only has effect for
	 * interpreted functions (those compiled with optimization level set to -1).
	 * As the interpreter doesn't use the Java stack but rather manages its own
	 ...
	 * 
	 * @param max
	 *            the new maximum interpreter stack depth
	 * @throws IllegalStateException
	 *             if this context's optimization level is not -1
	 * @throws IllegalArgumentException
	 ...
	 *             if there is already a SecurityController object for this
	 *             Context or globally installed.
	 ...
	 * is disassociated from the thread the values cannot be retrieved. Also, if
	 ...
	 * 
	 * @param debugger
	 *            the debugger to be used on callbacks from the engine.
	 * @param contextData
	 *            arbitrary object that debugger can use to store per Context
	 *            data.
	 */
	 ...
	 * {@link ContextFactory#hasFeature(Context cx, int featureIndex)} that
	 * allows to customize Context behavior without introducing Context
	 * subclasses. {@link ContextFactory} documentation gives an example of
	 ...
	 * instruction counting is disabled, otherwise each time the run-time
	 * executes at least the threshold value of script instructions,
	 ...
	 * between compiled and interpretive modes: executing a given script or
	 * function in the different modes will result in different instruction
	 * counts against the threshold. {@link #setGenerateObserverCount} is called
	 * with true if <code>threshold</code> is greater than zero, false
	 * otherwise.
	 * 
	 * @param threshold
	 ...
	 * Context subclasses. Run-time calls this when instruction counting is
	 * enabled and the counter reaches limit set by
	 ...
	 *            amount of script instruction executed since last call to
